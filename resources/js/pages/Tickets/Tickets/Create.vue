...
const props = defineProps<{
    users: { persons: { id: number; first_name: string; last_name_paternal: string; last_name_maternal: string } }
    ...
    <AppLayout>
        <div class="container mx-auto max-w-2xl px-4 py-8">
            ...
            <form @submit.prevent="submit" class="space-y-8 rounded-lg border bg-white p-8 shadow-sm dark:bg-gray-800">
                <div>
                    <Label for="issue_id" class="mb-2 block">Incidencia</Label>
                    <Select v-model="form.issue_id" required>
                        <SelectTrigger>
                            <SelectValue placeholder="Selecciona el tipo de incidencia" />
                        </SelectTrigger>
                        <SelectContent>
                            <SelectItem v-for="issue in props.issues" :key="issue.id" :value="issue.id.toString()">
                                {{ issue.name }}
                            </SelectItem>
                        </SelectContent>
                    </Select>
                    <div v-if="form.errors.issue_id" class="mt-1 text-sm text-red-600">
                        {{ form.errors.issue_id }}
                    </div>
                </div>

                <div>
                    <Label for="users" class="mb-2 block">Usuario Solicitante</Label>
                    <Select v-model="form.user_id" required>
                        ...
                        <SelectContent>
                            <SelectItem v-for="user in props.users" :key="user.id" :value="user.id.toString()">
                                ...
                                {{ user.person.last_name_paternal }}
                                {{ user.person.last_name_maternal }}
                            </SelectItem>
                        </SelectContent>
                    </Select>
                ...
                <div>
                    <Label for="area_id" class="mb-2 block">Área Usuaria</Label>
                ...
                <div>
                    <Label for="attachment" class="mb-2 block">Adjuntar Archivos (Opcional)</Label>
